try:
    ...
    class pdns(object):
        ...
        def resolve(self, qnames, where, port=53, rdtype=rdatatype.A):
            ...
            self.itime = time()   # init time: timestamp we started resolving
        ...
        def _wait(self, timeout):
            ...
            while timeout > 0:
                ...
                timeout = timeout - (time() - _stime)

        def _wait_timeslice(self):
            # comput how long we need to wait
            timeout = self.timeslice - (time() - self.stime)
            if timeout > 0:
                self._wait(timeout)
            # reset state
            self.stime = 0
        ...
        def _wait_for_outstanding_query(self):
            ...
            while True:
                ...
                timeout = self.timeout - (self.stime - time())
                # wait...
        ...
        def _handle_reply(self, q, r, stime, etime, address):
            if r:
                print 'dns_resolve_parallel: OK -1 -1', time(), etime - stime, address
                print r.to_text()
